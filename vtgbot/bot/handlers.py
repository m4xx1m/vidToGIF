...
from vtgbot.misc import db, dp
from vtgbot.log import logger
from vtgbot.convert import process_video
...
async def start(message: Message):
    ...
    await message.reply(
        "Hello!\nI can help you to remove sound from your video and transcode to AVC!\n"
        "Send me a video file and I'll do the magic!"
    )


async def _send_upload(bot, chat_id):
    for _ in range(10):
...
async def handle_video(message: Message):
    ...
    if not message.video and (not message.document or "video" not in message.document.mime_type):
        ...
        upload_task = asyncio.get_event_loop().create_task(_send_upload(message.bot, message.from_user.id))
        ...
        try:
            ...
            completed_fd, completed_vid_temp_path = mkstemp(prefix='COMPLETED_VID', suffix=".mp4")
            ...
            with open(completed_vid_temp_path, "rb") as completed_file:
                await message.reply_animation(completed_file)
